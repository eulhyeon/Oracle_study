...
(
    ...
    FROM DUAL
            ...
            SELECT :YYYYMM                                                                 AS END_DT
                   ...
                   ,TO_CHAR(ADD_MONTHS(TRUNC(DT_DATE,'Q'),2), 'YYYYMM')                    AS QTR_END_DT
                   ,TO_CHAR(TRUNC(TRUNC(DT_DATE,'Q')-1,'Q'), 'YYYYMM')                     AS LAST_QTR_START_DT
                   ,TO_CHAR(TRUNC(DT_DATE,'Q')-1, 'YYYYMM')                                AS LAST_QTR_END_DT
                   ,TO_CHAR(ADD_MONTHS(TRUNC(DT_DATE,'YY'), -12), 'YYYYMM')                AS LAST_YR_START_DT
                   ,TO_CHAR(ADD_MONTHS(DT_DATE,-12), 'YYYYMM')                             AS LAST_SAME_MON_DT
                   ,TO_CHAR(TRUNC(DT_DATE,'YY')-1, 'YYYYMM')                               AS LAST_YR_END_DT
                   ,TO_CHAR(ADD_MONTHS(TRUNC(DT_DATE,'Q'), -12), 'YYYYMM')                 AS LAST_YR_QTR_START_DT
                   ,TO_CHAR(ADD_MONTHS(ADD_MONTHS(TRUNC(DT_DATE,'Q'), -12),2), 'YYYYMM')   AS LAST_YR_QTR_END_DT
            FROM BASED_DT
        )
...
FROM(
        SELECT :YYYYMM YYYYMM
              ,T1.*
              ,SUM(T1.당월판매량) OVER(PARTITION BY T1.MS_GROUP) AS 제품당월판매량
              ,SUM(T1.전월판매량) OVER(PARTITION BY T1.MS_GROUP) AS 제품전월판매량
              ,SUM(T1.당월판매량) OVER(PARTITION BY T1.CUST_ID) AS 고객당월판매량
              ,SUM(T1.전월판매량) OVER(PARTITION BY T1.CUST_ID) AS 고객전월판매량
        FROM(
                SELECT MS_ID
                      ,SITE_ID
                      ,CUST_ID
                ...
                WHERE YYYYMM BETWEEN DT.LAST_YR_START_DT AND DT.END_DT -- 작년 1월부터 현재까지의 데이터만 조회
                GROUP BY MS_ID
                        ,SITE_ID
                        ,CUST_ID
                        ,MS_GROUP
...
SELECT :YYYYMM YYYYMM
        ...
        FROM (
                ...
                SELECT MS_ID
                      ...
                      ,SUM(CASE WHEN YYYYMM >= DT.RECENT_START_DT    AND YYYYMM <= DT.END_DT THEN QUANTITY ELSE 0 END) AS 최근판매량
                      ,SUM(CASE WHEN YYYYMM >= DT.QTR_START_DT       AND YYYYMM <= DT.END_DT THEN QUANTITY ELSE 0 END) AS 분기판매량
                      ,SUM(CASE WHEN YYYYMM >= DT.LAST_QTR_START_DT  AND YYYYMM <= DT.LAST_QTR_END_DT THEN QUANTITY ELSE 0 END) AS 전월분기판매량
                      ,SUM(CASE WHEN YYYYMM >= DT.RECENT_YR_START_DT AND YYYYMM <= DT.END_DT THEN QUANTITY ELSE 0 END) AS 연간최근판매량
                FROM FD_DP_MNLY_SHIPMNT, DT
